...
INSERT INTO `perfil` (`IdPerfil`, `DescripcionPerfil`) VALUES(1, 'Administrador'),(2,'Gato');
INSERT INTO `usuarios` (`NombreUsuario`, `ApellidosUsuario`, `usuario`, `contrasena`, `IdPerfil`) VALUES('Daniel', 'Santana', 'daniel.santana', '1e2c7d5589faf6a4a1863d503b50bc41', 1),('Yeraldin', 'Maldonado', 'Yeraldin.maldonado', '1bae1f2fa02e2be41c00da7282f2330c', 1),('David', 'Parrito', 'david.parrito', 'f182bb4f7003792e4b027d4fe5fbd9b3', 1),('Diego', 'Garcia', 'diego', 'd41d8cd98f00b204e9800998ecf8427e', 1),('Usuario', '', '', 'd41d8cd98f00b204e9800998ecf8427e', 1);



LOAD DATA
INFILE 'C:/ProgramData/MySQL/MySQL Server 8.0/Uploads/cliente.csv'
INTO TABLE cliente
...
-- iduser david.parrito - Intec@123.			
...
select * from perfil;
select * from catalogoestatuspaquetes;





SELECT `usuarios`.*, date_format(fechaActualizado,"%m") as fechaM, date_format(fechaActualizado,"%y") as fechaA,`DescripcionPerfil`, `perfil`.`IdPerfil` FROM `usuarios` JOIN `perfil` ON `usuarios`.`IdPerfil` = `perfil`.`IdPerfil` 
WHERE `usuario` = 'diego' AND `contrasena` = 'b29fcab227778f45f7cab8e6d1f5e911' LIMIT 1;
update usuarios set fechaActualizado = '2018-06-05 22:05:15' WHERE `IdUsuario` = '1';
UPDATE `usuarios` SET fechaCaducidad = ADDDATE(fechaActualizado, INTERVAL 180 DAY) where IdUsuario = 5;
UPDATE `usuarios` SET fechaCaducidad = '2018-06-06 22:05:19' where IdUsuario = 6;
select * from paquete_envio;


update paquete_envio set IdEstatusPaquete= 6 where IdPaqueteEnvio = 1;

select  count(*) from paquete_envio where IdEstatusPaquete < 6;


select  count(*) from equipo_orden where IdEstatusPaquete < 3 and IdOrden = 1;

select * from paquete_envio;



select count(*) as retardo
from equipo_orden 
inner join paquete_envio on paquete_envio.IdPaqueteEnvio = equipo_orden.IdPaqueteEnvio
inner join laboratorio on paquete_envio.IdLaboratorio = laboratorio.IdLaboratorio 
where equipo_orden.IdEstatusPaquete = 3 and ADDDATE(equipo_orden.FechaRetLab, INTERVAL diasServicios DAY) < now() ; 

select * from equipo_orden;

update equipo_orden set IdEstatusPaquete = 3 where IdEquipoOrden = 2;


select *,equipo_orden.IdOrden as IdOrdenes
from equipo_orden 
inner join paquete_envio on paquete_envio.IdPaqueteEnvio = equipo_orden.IdPaqueteEnvio
inner join laboratorio on paquete_envio.IdLaboratorio = laboratorio.IdLaboratorio
inner join equipo on equipo.IdEquipo = equipo_orden.IdEquipo 
inner join cliente on cliente.IdCliente = equipo.IdCliente 
where equipo_orden.IdEstatusPaquete = 3 and ADDDATE(equipo_orden.FechaRetLab, INTERVAL diasServicios DAY) < now() ; 



SELECT * 
FROM `usuarios` JOIN `perfil` ON `usuarios`.`IdPerfil` = `perfil`.`IdPerfil`
 WHERE `usuario` = ''and fechaActualizado < now()  LIMIT 1;
...
select * from equipo_orden 
inner join paquete_envio on paquete_envio.IdPaqueteEnvio = equipo_orden.IdPaqueteEnvio
inner join laboratorio on laboratorio.IdLaboratorio = paquete_envio.IdLaboratorio
where equipo_orden.IdPaqueteEnvio = 1;

select equipo_orden.IdOrden,paquete_envio.FechaEnv,paquete_envio.Descripcion,laboratorio.* from equipo_orden 
inner join paquete_envio on paquete_envio.IdPaqueteEnvio = equipo_orden.IdPaqueteEnvio
inner join laboratorio on laboratorio.IdLaboratorio = paquete_envio.IdLaboratorio
where equipo_orden.IdPaqueteEnvio = 1;
...
select * from paquete_envio 
inner join laboratorio on laboratorio.IdLaboratorio = paquete_envio.IdLaboratorio
where paquete_envio.IdPaqueteEnvio = 1;




select *,equipo_orden.IdOrden as ID from equipo_orden 
inner join paquete_envio on paquete_envio.IdPaqueteEnvio = equipo_orden.IdPaqueteEnvio
inner join equipo on equipo_orden.IdEquipo = equipo.IdEquipo
inner join cliente on cliente.IdCliente = equipo.IdCliente
where equipo_orden.IdPaqueteEnvio = 1;

SELECT equipo.*,cliente.NombreCompania,`equipo_orden`.`IdOrden` as `ID` FROM `equipo_orden` 
